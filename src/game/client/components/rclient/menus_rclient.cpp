...
void CMenus::RenderSettingsRushieSettings(CUIRect MainView)
{
	...
	Column.HSplitTop(MarginSmall, nullptr, &Column);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClNullMovement, RCLocalize("Null movement/ Snap tap"), &g_Config.m_ClNullMovement, &Column, LineSize);
	GameClient()->m_Tooltips.DoToolTip(&g_Config.m_ClNullMovement, &Column, RCLocalize("To see that hold D and then hold A"));
	Column.HSplitTop(MarginSmall, nullptr, &Column);
